...
def get_all_indexs
  ...
  index_aplhabets.each do |li_element|
    ...
    li_element.css('ul > li').each do |sub_li_element|
      ...
      array_of_li << { index_aplhabet: link.text.downcase, url: BASE_URL+link.attribute('href').value} if link.text.downcase.include?('-') || link.text.downcase.size<2
    end
    uk_london_index_aplhabets.push(li_element.css('h4').text.downcase => array_of_li)
  end
  puts uk_london_index_aplhabets
end
...
def scrape_single_property(property_link)
  # Selenium::WebDriver::Chrome.driver_path = "/home/touqeer/Desktop/rightmove/chromedriver"
  p property_link
  ...
  p '*'*100
  p @browser.div(:class => "ajaxLoadingSpinner").present?
  ...
  puts latitude = str.split("latitude:").last.to_f
  puts longitude = str.split("longitude:").last.to_f
  ...
  puts postal_code = location.postal_code
  puts road = location.route
  puts city = location.city
  puts country = location.country
  puts state = location.state
  puts listed_on = nokogiri_page.css("div#firstListedDateValue").text
  puts asking_price = nokogiri_page.css("p#propertyHeaderPrice.property-header-price > strong").text.strip.gsub(" for sale", "")
  puts description = nokogiri_page.css("h1.fs-22").text.strip
  puts beds = description.downcase.include?("bedroom") ? description.to_i : "-"
  ...
  if history.text.strip == ""
    ...
    puts last_sold_price = history[1].text.strip
    puts last_sold_date = history[0].text.strip
  end
...
def scrape_properties_from_single_page(nokogiri_page, city, property_for, properties_per_page)
...
# url = "http://www.rightmove.co.uk/property-for-sale/property-61494193.html"
# scrape_single_property(url)
# url = "http://www.rightmove.co.uk/property-for-sale/property-72583241.html"
# scrape_single_property(url)
# url = "http://www.rightmove.co.uk/property-for-sale/property-67753688.html"
# scrape_single_property(url)
# url = "http://www.rightmove.co.uk/property-for-sale/property-62602039.html"
# scrape_single_property(url)
# url = "http://www.rightmove.co.uk/property-for-sale/property-72595028.html"
# scrape_single_property(url)
# url = "http://www.rightmove.co.uk/property-for-sale/property-71126420.html"
# scrape_single_property(url)
url = "http://www.rightmove.co.uk/property-for-sale/property-68167445.html"
scrape_single_property(url)
# url = "http://www.rightmove.co.uk/property-for-sale/property-52681605.html"
# scrape_single_property(url)